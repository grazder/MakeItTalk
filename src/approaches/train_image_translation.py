...
class Image_translation_block:
        ...
        if not single_test:
            ...
            if opt_parser.use_vox_dataset == "raw":
                if opt_parser.comb_fan_awing:
                    ...
                    from src.dataset.image_translation.image_translation_dataset import (
                        image_translation_raw98_with_audio_dataset as image_translation_dataset,
                    )
                else:
                    from src.dataset.image_translation.image_translation_dataset import (
                        image_translation_raw98_dataset as image_translation_dataset,
                    )
            else:
                from src.dataset.image_translation.image_translation_dataset import (
                    image_translation_preprocessed98_dataset as image_translation_dataset,
                )
            ...
            self.optimizer = torch.optim.Adam(
                self.G.parameters(), lr=opt_parser.lr, betas=(0.5, 0.999)
            )
    ...
    def __train_pass__(self, epoch, is_training=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            if self.opt_parser.comb_fan_awing:
                ...
                fan_pred_landmarks = (
                    fan_pred_landmarks.reshape(-1, 68, 3).detach().cpu().numpy()
                )
            ...
            with torch.no_grad():
                ...
                image_in, image_out = image_in.reshape(-1, 3, 256, 256).to(
                    device
                ), image_out.reshape(-1, 3, 256, 256).to(device)
                inputs = image_out
                ...
                if self.opt_parser.comb_fan_awing:
                    ...
                    pred_landmarks = np.concatenate(
                        [fl_jaw_eyebrow, fl_rest], axis=1
                    ).astype(np.int)
            ...
            if self.opt_parser.write:
                ...
                self.writer.add_scalar(
                    "loss_l1", loss_l1.cpu().detach().numpy(), self.count
                )
                self.writer.add_scalar(
                    "loss_vgg", loss_vgg.cpu().detach().numpy(), self.count
                )
                self.count += 1
            ...
            if i % self.opt_parser.jpg_freq == 0:
                vis_in = np.concatenate(
                    [
                        image_in[0, 3:6].cpu().detach().numpy().transpose((1, 2, 0)),
                        image_in[0, 0:3].cpu().detach().numpy().transpose((1, 2, 0)),
                    ],
                    axis=1,
                )
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            for j in range(image_in.shape[0] // 16):
                ...
                for k in range(g_out.shape[0]):
                    frame = (
                        np.concatenate(
                            (ref_in[k], g_out[k], fls_in[k], ref_out[k]), axis=1
                        )
                        * 255.0
                    )
    ...
    def single_test(
        ...
        final_width = 256 if get_final_video_only else 256 * 3
        writer = cv2.VideoWriter(
            "out.mp4", cv2.VideoWriter_fourcc(*"mjpg"), 62.5, (final_width, 256)
        )
        ...
        for i, frame in enumerate(fls):
            ...
            image_in, image_out = torch.tensor(
                image_in, requires_grad=False
            ), torch.tensor(image_out, requires_grad=False)

            image_in, image_out = image_in.reshape(-1, 6, 256, 256), image_out.reshape(
                -1, 3, 256, 256
            )
            image_in, image_out = image_in.to(device), image_out.to(device)
            ...
            g_out[g_out < 0] = 0
            ref_in = (
                image_in[:, 3:6, :, :].cpu().detach().numpy().transpose((0, 2, 3, 1))
            )
            fls_in = (
                image_in[:, 0:3, :, :].cpu().detach().numpy().transpose((0, 2, 3, 1))
            )
        ...
        os.system(
            "ffmpeg -loglevel error -y -i out.mp4 -i {} -pix_fmt yuv420p -strict -2 examples/{}_{}.mp4".format(
                "examples/" + filename[9:-16] + ".wav", prefix, filename[:-4]
            )
        )
